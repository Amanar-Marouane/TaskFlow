...
class Basic
{
    ...
    public function render($status)
    {
        ...
        foreach ($rows as $row) {
            if ($row['task_type'] == "Bug") $svg = $bug;
            elseif ($row['task_type'] == "Basic") $svg = $basic;
            else $svg = $feature;
            echo "<div class='bg-white border border-gray-300 rounded shadow p-4 flex flex-col gap-3'>
                    <div class='flex items-center'>
                        $svg
                        <h3 class='font-semibold'>{$row['task_title']} ({$row['importance']})</h3>
                    </div>
                    <p class='text-sm text-gray-600'>" . substr($row['task_description'], 0, 20) . "...</p>
                    <a href='./../controllers/Controller.php?action=show&id={$row['task_id']}' class='text-sm text-[#007bff]'>View details</a>
                    <div class='flex items-center space-x-2 mt-2'>
                        <span class='inline-block w-4 h-4 bg-green-500 rounded-full' title='{$row['assigned_to']}'></span>
                        <span class='text-sm text-gray-600'>Assigned to: {$row['assigned_to']}</span>
                    </div>
                    <div class='flex items-center space-x-2 mt-2'>
                        <span class='inline-block w-4 h-4 bg-green-500 rounded-full' title='{$row['task_status']}'></span>
                        <span class='text-sm text-gray-600'>Current status: {$row['task_status']}</span>
                    </div>
                    <p class='text-xs text-gray-500'>Created at: {$row['created_at']}</p>
                </div>";
        }
    }
}
